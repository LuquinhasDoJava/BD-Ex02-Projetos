...
ALTER TABLE projects DROP CONSTRAINT CONSTRAINT_DATA; --Tirando constraint check do campo 'data'
...
SELECT nome, descricao, data_projeto, DATEADD(DAY,15, data_projeto) AS data_final, 'aparecido@empresa.com'  AS email
FROM projects
WHERE id = 10001;
...
FROM 
    projects
WHERE 
    nome LIKE '%Manutenção%' AND  -- Filtra projetos que contêm "Manutenção"
    data_projeto <= '2014-09-16';  -- Garante que a data do projeto é anterior ou igual à data final
